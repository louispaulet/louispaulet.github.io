...
const PostTile = ({ id, title, summary, postDate, image, imageAlt, tags = [] }) => {
  return (
    <Link to={`/posts/${id}`} className="group flex h-full min-w-0 w-full">
      <article className="tile-interactive flex h-full min-w-0 w-full flex-col p-4 text-secondary sm:p-6">
        {image ? (
        ...
        <h3 className="mt-5 min-w-0 text-xl font-semibold leading-snug text-primary transition-colors duration-300 group-hover:text-accent">
          {title}
        </h3>
